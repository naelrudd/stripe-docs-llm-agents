...
For further assistance, contact your implementation partner or [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D).
...
3. Verify the bank deposit account exists and note its internal ID.
4. Check that the **Stripe Limited Access** role has permission to view the account.
5. In the Stripe Connector for NetSuite app, go to **App settings**.
6. Navigate to the **Account Mapping** tab and update the bank deposit account mapping with the correct internal ID.
...
2. Review the list of transactions with sync issues.
3. For each unsynced transaction, check its sync status and error message.
4. Resolve any errors preventing the sync (such as missing customer or account mapping issues).
5. Retry the sync if needed. After all transactions sync, the payout deposit automatically proceeds.
...
2. In the connector app, go to **App settings** > **Accounts mapping**.
3. Configure the required account mapping and save your changes.
4. Retry the payout sync.
...
2. Find the **Bank deposit accounts** section.
3. Add a bank deposit account mapping for the payout’s currency.
4. Save your changes and retry the payout sync.
...
2. Edit the Stripe Limited Access role.
3. Under the **Subsidiaries** tab, make sure the role has access to all required subsidiaries.
4. Verify the **Lists** > **Subsidiaries** permission is set to at least View.
5. Save the role changes and retry the payout sync.
...
2. If the error persists, log in to NetSuite as an administrator and navigate to **Setup** > **Users/Roles** > **Manage Roles**.
3. Verify the Stripe Limited Access role has **Lists** > **Subsidiaries** permission set to Full and **Reports** > **Search** permission enabled.
4. Check that subsidiaries are properly configured on the bank deposit account.
5. Contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) if the issue continues.
...
2. Search for your undeposited funds account and verify it exists and is active.
3. Check that the Stripe Limited Access role has permission to view the account.
4. In the connector app, go to **App settings** > **Accounts mapping** and update the undeposited funds account mapping if needed.
5. Retry the payout sync.
...
1. In the connector app, go to **App settings** > **Accounts mapping**.
2. Find the **Undeposited Funds Account** setting.
3. Select the appropriate undeposited funds account from your NetSuite chart of accounts.
4. Save your changes and retry the payout sync.
...
No action required. The connector automatically retries with exponential backoff. If the error persists for more than 24 hours, contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D).
...
1. In the connector app, navigate to the payout details page.
2. Review the list of transactions with validation errors.
3. Each transaction shows its specific validation error (for example, amount mismatch, already deposited, deleted record, or invalid subsidiary).
4. Resolve each validation error following the guidance in the [Payout transaction validation](https://docs.stripe.com/use-stripe-apps/netsuite/error-resolution.md#payout-transaction-validation) section.
5. After you resolve all validation errors, the payout deposit automatically proceeds.
...
1. Log in to your NetSuite account.
2. Navigate to one of the `already_deposited` customer payments.
3. Verify that the payment’s account is set to **Undeposited Funds**.
...
4. Retry the payout sync.
5. If the record was correctly deposited in a previous payout:
   - The Stripe transaction might need to be excluded from payout syncing.
   - Contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) for assistance reconciling.
...
2. If the deletion is unintentional, manually recreate the record in NetSuite with the correct amount and details.
3. Contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) to relink the Stripe transaction to the correct NetSuite record.
4. Retry the payout sync.
...
2. Determine which Stripe transaction should remain linked to the existing NetSuite record.
3. For the other transactions:
   - Create new NetSuite records in NetSuite matching the Stripe transaction details.
4. Use the manual override tool in the SCN app drawer to link the transaction to the correct NetSuite record.
5. If this doesn’t work, contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) to update the record linkages.
6. Retry the payout sync.
...
2. Check the **Subsidiary** field on the record.
3. Compare it to the subsidiaries configured on your bank deposit account.
4. Either edit the NetSuite record to use a valid subsidiary, or update the bank deposit account to include this subsidiary.
5. Changing subsidiaries on records might have accounting implications. Consult your accounting team.
6. Retry the payout sync.
...
1. Log in to NetSuite and find the NetSuite record linked to this transaction.
2. Check the **Account** field on the record’s line item.
3. Compare it to the undeposited funds account in your connector settings.
4. Either edit the NetSuite record to use the correct undeposited funds account, or update your connector settings to match the account used on records.
5. Changing accounts on records might require journal entries to correct. Consult your accounting team.
6. Retry the payout sync.
...
2. Log in to NetSuite and find the NetSuite record linked to this transaction.
3. Compare the **Total** or **Amount** field with the Stripe amount.
4. Identify the source of the discrepancy by checking System Notes for manual edits, or reviewing workflow and script logs for automated changes.
5. Either correct the NetSuite record to match the Stripe amount, or if the NetSuite amount is intentionally different, contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) for guidance.
6. Retry the payout sync.
...
1. Log in to NetSuite as an administrator and navigate to **Setup** > **Users/Roles** > **Manage Roles**.
2. Edit the Stripe Limited Access role.
3. Verify the role has **Transactions** > **Make Deposits** permission set to at least View.
4. Check that **Reports** > **Search** permission is enabled.
5. Retry the payout sync. If the error persists, contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D).
...
For these errors, contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D) with your payout ID (for example, `po_xxx`) and the error code.
...
| `scn_ns_record_validation_error` | A NetSuite record validation error occurred. This typically means a required field on the invoice, sales order, or customer is missing or has an invalid value. To identify any blocking fields, open and attempt to save the invoice or sales order and the customer in the NetSuite UI. For created records (such as customer payment or customer deposit), you can set default values for required fields by adding a static mapping in the Connector app **Settings**\>**Field Mapping**. |
...
| `scn_unknown_error` | An unknown error occurred. Contact [Stripe support](https://support.stripe.com/contact/email?topic=stripe_apps&app=netsuite&subject=%5BStripe%20Connector%20for%20NetSuite%20\(SCN\)%5D). |
...
3. Paste one or more Stripe resource IDs, separated by commas. The maximum number of IDs you can submit at once is shown in the UI.
